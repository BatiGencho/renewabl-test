...
fn calculate_optimal_pool_size(
    ...
    // Leave some headroom for admin connections and other services
    let available =
    ...
    // Cap at a reasonable maximum pool size
...
pub async fn establish_connection(
    ...
    // NOTE: In a real deployment, we would determine num_app_instances
    // from our orchestration environment (e.g., Kubernetes, ECS, etc.)
    // also consider increasing MIN_RESERVED_CONNECTIONS based on observed needs
    // and increasing MAX_POOL_SIZE if we notice excessive connection churn (basically uncapping the max_pool_size)
    let max_pool_size =
...
pub async fn shutdown_pool_with_timeout(
    ...
    // If there are active connections, wait for them
    if state.connections > state.idle_connections {
        ...
        // Poll until all connections are idle or timeout
    ...
    // Drop the pool to close all connections
    drop(pool);
...
{
    // Get pool stats before acquiring
    let pool_state_before = pool.state();

    // Instrument connection acquisition with pool state
    ...
    // Track when connection is held and used
...
pub fn connection_error_to_diesel(
    ...
    match err {
        WithConnectionError::Pool(e) => {
            // Convert pool error to a database error
