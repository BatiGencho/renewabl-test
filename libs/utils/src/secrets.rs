...
// Helper function to detect if a value is an AWS Secrets Manager ARN
fn is_secrets_manager_arn(value: &str) -> bool {
    // ARN format: arn:aws:secretsmanager:region:account-id:secret:secret-name
...
pub async fn get_secret(name: &str) -> Result<String, Box<dyn Error>> {
    ...
    if is_secrets_manager_arn(&env_value) {
        ...
        // Value is not an ARN, use it directly (local development or direct value)
        tracing::debug!(
